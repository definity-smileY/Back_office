...
def refund(self, request, queryset):
    with transaction.atomic():
        ...
        for obj in qs:
            ...
            LogEntry.objects.log_action(#동작하는걸 추가하는 로그기능
...
class OrderAdmin(admin.ModelAdmin):
    ...
    def changeform_view(self, request, object_id=None, form_url='', extra_context=None):
        ...
        extra_cntext = { 'title' : f"'{order.fcuser.email}'의 '{order.product.name}' 주문 수정하기"}
        extra_cntext['show_save_and_add_another'] = False
        extra_cntext['show_save_and_continue'] = False
